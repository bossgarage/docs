...
O Honduino OBD2 DPFI é uma unidade de controlo do motor (ECU) aftermarket concebida especificamente para veículos Honda com sistemas DPFI (Dual Point Fuel Injection). Este guia orienta-o através da configuração completa, calibração e configuração avançada da versão V4 desta ECU.

Esta documentação abrange:

- Configuração inicial e instalação do software necessário
- Configuração dos parâmetros fundamentais da ECU (roda dentada, ignição, injeção)
- Ligação e calibração da ECU com o seu veículo
- Configuração de recursos avançados como sensor O2 wideband, controlo de arranque e controlo de boost
- Diagramas de pinagem de referência para ligação de entradas e saídas

Quer esteja a atualizar da ECU original ou a substituir uma unidade aftermarket existente, este guia fornece instruções passo a passo para fazer o seu Honda funcionar com desempenho ideal utilizando o Honduino OBD2 DPFI V4.

## Antes de Ligar a ECU ao Carro
